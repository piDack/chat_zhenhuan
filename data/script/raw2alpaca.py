...
def parse_dialogue(dialogue,lines):
    for i in range(1,len(lines)-1):
        ...
        if re.match(r'^\w+:+?', line) and re.match(r'^\w+:+?', last_line):
            ...
            dialogue.append({'p': p_dialogue,'c':character, 'l': l_dialogue})
    return dialogue
...
if __name__ == '__main__':
    files = ['甄嬛传剧本01-10.txt','甄嬛传剧本11-20.txt','甄嬛传剧本21-30.txt','甄嬛传剧本31-40.txt','甄嬛传剧本41-50.txt','甄嬛传剧本51-60.txt','甄嬛传剧本61-70.txt','甄嬛传剧本71-76.txt']
    dialogue = []
    path = '../raw/'
    ...
    write_json(dialogue, 'output.json')
